  ...
  <div id="appSettings">
    ...
    <div class="recommend">
      <div class="current-recommend">
        <Card>
          <p slot="title">当前置顶设置：</p>
          <p v-for="(item,index) in topProposals" class="current-recommend-list">
            {{item.title+' (ID：'+item.id+')'}}
            <Icon type="close-round" class="remove-commend" @click.native="removeTop(item.id)"></Icon>
          </p>
        </Card>
      </div>
      <div>
        ...
        <Input style="width: 200px" v-model="topId"/>
        <Button type="primary" @click="proposalDetailHandle(topId,'top')">查询</Button>
      </div>
      <proposal-result :result="topProposalResult"></proposal-result>
      <div v-if="topProposalResult" class="set-top-btn">
        <Button type="primary" :disabled="topProposalResult.top == 1" @click="setTop(topProposalResult.id)">设为置顶</Button>
        <Button type="error" v-if="topProposalResult.top" @click="removeTop(topProposalResult.id)">移除</Button>
      </div>
    </div>
    ...
    <div class="recommend">
      ...
      <div>
        ...
        <Button type="primary" @click="proposalDetailHandle(recommendId,'hot')">查询</Button>
      </div>
      <proposal-result :result="hotProposalResult"></proposal-result>
      <div v-if="hotProposalResult" class="set-top-btn">
        <Button type="primary" :disabled="hotProposalResult.hot == 1" @click="hotProposal(hotProposalResult.id)">设为推荐</Button>
    ...
    export default {
      data () {
        return {
          ...
          hotProposals: [], //热门推荐
          topProposals: [], //置顶设置
          topProposalResult: null, //置顶设置查询结果
      ...
      methods: {
        ...
        proposalDetailHandle (pid,type) {
          ...
          proposalDetail({id:pid}).then(res => {
            if(Object.keys(res.data).length === 0){
              ...
              type === 'top' ? this.topProposalResult = null : this.hotProposalResult = null;
              return;
            }
            type === 'top' ? this.topProposalResult = res.data : this.hotProposalResult = res.data;
          })
        },
        //查看大图
        bannerPreview (url) {
        ...
        //置顶设置
        setTop (id) {
          if(this.topProposals.length === 3){
            this.$Message.error('最多设置3个置顶提案！');
            return
          }
          topProposal({
            proposalId: id
              ...
              this.topProposals.push(this.topProposalResult);
          })
        },
        //移除置顶
        removeTop (id) {
          this.$Modal.confirm({
            ...
            onOk: () => {
              removeTopProposal({proposalId:id}).then(res => {
                this.topProposalResult = null;
                let r_index;
                this.topProposals.forEach((e,i) => {
                  e.id === id && (r_index = i)
                });
                this.topProposals.splice(r_index,1);
              })
            },
            onCancel: () => {
      ...
      mounted () {
        ...
        getAppSettingData().then(res => {
          ...
          this.topProposals = res.data.topProposalIds;
